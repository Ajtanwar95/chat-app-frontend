...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  useEffect(() => {
    socket = io(ENDPOINT); //set up socket io connection
    socket.emit("setup", user); //providing user data as the payload
    socket.on("connected", () => setSocketConnected(true)); // When the client successfully connects to the Socket.io server, this event is triggered,update the component's state, indicating that the socket is connected.
  ...
  const typingHandler = (e) => {
    ...
    if (!socketConnected) return; //avoid performing typing animation when the socket is not connected.
